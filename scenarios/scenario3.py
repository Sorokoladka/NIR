...
# СЦЕНАРИЙ 3
# Влияние трудовых рисков на долгосрочные программы
...
if __name__ == '__main__':
    ...
    # 3.2 для ИИС беру 3 вариант (20/80)
    for (stock, bond) in [(0.2,0.8)]:
    ...
    salary_models = {'none':None}
    stochastic_salary_model = StochasticSalaryModel(
        initial_age=age
    )
    salary_models['stochastic'] = stochastic_salary_model

    for annual_growth in np.arange(1,10) / 100:
        salary_models[f'determenistic-{round(annual_growth, 2)}']\
            = DeterministicSalaryModel(annual_growth=annual_growth)

    unemployment_models = {'none':None}
    weibull_unemployment_model = WeibullUnemploymentModel( #or None
        p_exit = unemployment_p,
        weibull_k = unemployment_k,
        weibull_lambda = unemployment_lambda
    )
    unemployment_models['weibull'] = weibull_unemployment_model
    ...
    for scenario in list(simulated_returns.keys()):
        ...
        for salary_model_key, salary_model in salary_models.items():
            for unemployment_model_key, unemployment_model in unemployment_models.items():
                ...
                for i in range(n_simulations):
                    ...
                    if 'pds' in scenario:
                        ...
                        result_dict_temp[f'{salary}_{salary_model_key}_{unemployment_model_key}_{i}'] = program_calculator.compute_metrics()
    ...
    rows = [
        {
            'scenario_n': scenario,
            'salary_salary_model_unemployment_model_i': salary,
            'metric_name': metric,
            'metric_value': value
        }
    ...
    pre_df['salary'] = pre_df['salary_salary_model_unemployment_model_i'].apply(lambda x: int(x.split('_')[0]))
    pre_df['salary_model'] = pre_df['salary_salary_model_unemployment_model_i'].apply(lambda x: x.split('_')[1])
    pre_df['unemployment_model'] = pre_df['salary_salary_model_unemployment_model_i'].apply(lambda x: x.split('_')[2])
    pre_df['i'] = pre_df['salary_salary_model_unemployment_model_i'].apply(lambda x: int(x.split('_')[-1]))
