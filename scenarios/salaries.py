...
if __name__ == '__main__':
    ...
    simulated_returns[f'pds_{n}'] = np.full(n, 0.01)
            ...
            for age in age_range:
                ...
                for i in range(n_simulations):
                    ...
                    if 'pds' in scenario:
                        ...
                        result_dict_temp[f'{salary}_{age}_{i}'] = program_calculator.annual_salaries[-2]/12
    ...
    rows = [
        ...
        for salary, value in salaries.items()
    ]
    ...
    pre_df['salary'] = pre_df['salary_age_i'].apply(lambda x: int(x.split('_')[0]))
    pre_df['age'] = pre_df['salary_age_i'].apply(lambda x: int(x.split('_')[1]))
    pre_df['i'] = pre_df['salary_age_i'].apply(lambda x: int(x.split('_')[-1]))
    df = pre_df.drop(columns=['scenario_n','salary_age_i'])
